...
//作品类型
initpid(111, $("#work-type"))
//题材类型
initpid(110, $("#theme-type"))
//作品登机构
...
function bindEvents(){
	...
	$doc.on("click", ".next-step", function(){		
		...
		let $workNameVal = $("#work-name").val(),
			...
			$joint_producer = $("#joint_producer").val(),
			$creator_name = $("#creator_name").val(),
			$creator_type = $("#creator_type").val(),
			$storage_medium = $("#storage_medium").val(),
			$storage_medium_size = $("#storage_medium_size").val(),
			// $albumVal = $("#album").val(),
		...
		if($this.hasClass("next-step-01")){
			//第一步			
			if(util.isEmpty($workNameVal)){
				util.showMsg("作品名称不能为空！")
				return;
			}
			if(util.isEmpty($workTypeVal)){
				util.showMsg("作品类型不能为空！")
				return;
			}
			if(util.isEmpty($themeTypeVal)){
				util.showMsg("作品题材不能为空！")
				return;
			}
			if(util.isEmpty($lenghtTimeVal)){
				util.showMsg("作品时长不能为空！")
				return;
			}
			nextBtn($this, $step);
		}else if($this.hasClass("next-step-02")) {
			...
			//}else{
				...
				let da = {
					...
					storage_medium_size: $storage_medium_size,
	...
	$doc.on("click", ".btn-List-qr", function(){
		...
		if(util.isEmpty($modalObligeeNameVal)){
			util.showMsg("权利人名称不能为空！")
			return
		}
		if(util.isEmpty($modalObligeeTypeVal)){
			util.showMsg("权利人类型不能为空！")
			return
		}
			let $id = $(this).attr("id");
			let jsond = {
	...
	$.ajax({
		...
		success: function(res) {
			if(res && res.status == 1){
				...
				$("#producer").val(data.opus.producer);
				$("#joint_producer").val(data.opus.joint_producer);
				$("#creator_name").val(data.opus.creator_name);
				$("#creator_type").val(data.opus.creator_type);
				$("#storage_medium").val(data.opus.storage_medium);
				$("#storage_medium_size").val(data.opus.storage_medium_size);
...
// 导出文件
$("#exportMoviesBtn").click(function() {
	util.export(5)
});	
