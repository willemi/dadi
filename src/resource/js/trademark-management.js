...
function bindEvents(){
	...
	$doc.on("click", ".btn-xz-qr", function(){
		...
		if(util.isEmpty($mark_name) || util.isEmpty($mark_progress) || util.isEmpty($mark_type) || util.isEmpty($mark_register_leibie) || util.isEmpty($mark_register_type)){
			util.showMsg("申请进度，商标名称、商标类别，注册类别，注册类型不能为空！")
			return
		}else {
